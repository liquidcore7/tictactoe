...
class BotService(WithDataBase[List[TicTacToeDelta.TicTacToeDeltaDTO]]):
    ...
    def update_session(self, session_id: str, with_value: TicTacToeDelta.TicTacToeDeltaDTO) \
        ...
        if with_value.player != TicTacToeHelper.Players.FREE:
            ...
            if TicTacToeModel.is_won(maybe_player_field):
        ...
        next_move: TicTacToeDelta.TicTacToeDeltaDTO = self.__next_move__(all_moves)

        self.__append__(session_id, next_move)
        resulting_field: TicTacToeModel.TicTacToeDTO = self.session_service.update_session(session_id, next_move)

        if TicTacToeModel.is_won(resulting_field):
            self.delete_session(session_id)
